...
if (isset($_GET['protein_id'])) {
    $category_id = $_GET['protein_id'];

    $stmtCategory = $conn->prepare("SELECT * FROM categories_product WHERE protein_id = ?");
    $stmtCategory->bind_param("i", $category_id);
    $stmtCategory->execute();
    $products = $stmtCategory->get_result();

    $stmtSpecific = $conn->prepare("SELECT * FROM specific_weight_products WHERE protein_id= ?");
